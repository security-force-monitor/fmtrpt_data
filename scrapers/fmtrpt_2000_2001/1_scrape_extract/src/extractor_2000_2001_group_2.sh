#!/bin/bash
#
# Parsing FMTRPT FY 2000-2001 (Group 2)
...
_trimLeadingTrailingItems () {
	# trim raw XML for particular Region to remove leading or trailing trainings from other regions which appear on those pages
	 case "${t}" in
		"Europe")
			sed "1,4d ; 4861,4889d"
			# prunes Algeria from end of PDF page 15
			;;
		"East_Asia_and_Pacific")
			sed "1,4d ; 10,199d"
			# 4047,4159d to remove Albania, but suggest we capture here Europe is separate PDF
			;;
		"Near_East")
			sed "1,4d; 10,251d ; 5954,6061d"
			# Starts region capture with Algeria on PDF page 15, chops off Armenai from PDF page 32
			;;
		"Newly_Independent_States")
			sed "1,4d ; 9,195d"
			# Includes first set of Bangladesh results at end of last page
			;;
		"South_Asia_Region")
			sed "1,4d ; 1247,1274d"
			# Chops off Antigua and Barbuda at end of PDF page 4
			;;
		"Western_Hemisphere_Region")
			sed "1,8d ; 10,284d"
			# Chops off material from South Asia Region on PDF page 4, starts at Antigua
			;;
		esac
}
...
_composeProgramTitle () {
	...
	perl -00pe 's/<text.*left="486".*height="11" font="1">(.*)<\/text>\n<text.*left="486".*height="11" font="1">(.*)<\/text>\n<text.*left="486".*height="11" font="1">(.*)<\/text>/<text_program_name>\1%\2%\3<\/text_program_name>/g ;
		    s/<text.*left="87".*height="11" font="1">(.*)<\/text>\n<text.*left="87".*height="11" font="1">(.*)<\/text>\n<text.*left="87".*height="11" font="1">(.*)<\/text>/<text_program_name>\1%\2%\3<\/text_program_name>/g ;
		    s/<text.*left="486".*height="11" font="1">(.*)<\/text>\n<text.*left="486".*height="11" font="1">(.*)<\/text>/<text_program_name>\1%\2<\/text_program_name>/g ;
	            s/<text.*left="87".*height="11" font="1">(.*)<\/text>\n<text.*left="87".*height="11" font="1">(.*)<\/text>/<text_program_name>\1%\2<\/text_program_name>/g ;
		    s/<text.*left="486".*height="11" font="1">(.*)<\/text>/<text_program_name>\1<\/text_program_name>/g ;
		    s/<text.*left="87".*height="11" font="1">(.*)<\/text>/<text_program_name>\1<\/text_program_name>/g'
}
...
_filterOutUselessLines () {
	...
	grep -e "^<text" \
	| grep -v -e "font=\"1\"><b>[0-9].*$"\
	| grep -v -e "font=\"1\"><b>.*Totals:.*$"\
	| grep -v -E "font=\"1\"><b>\\$.*<\/b>.*$"\
	| grep -v -e ">Cost<\/text>$" \
	| grep -v -e ">Title <\/text>$" \
	| grep -v -e ">of # <\/text>$"
...
_assignTrainingAttributes () {
	...
	sed -E '{
			# Left column 
			s/^.*left=\"87\".*font=\"2\">(.*)<\/text>$/<course_title>\1<\/course_title>/g
			s/^.*left=\"3[01][23458]\".*font=\"2\">(.*)<\/text>$/<quantity>\1<\/quantity>/g
			s/^.*left=\"3[5-9][0-9]\".*font="\2\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			# Right column
			s/^.*left=\"486\".*font=\"2\">(.*)<\/text>$/<course_title>\1<\/course_title>/g
			s/^.*left=\"7[01][23478]\".*font=\"2\">(.*)<\/text>$/<quantity>\1<\/quantity>/g
			s/^.*left=\"7[5-9][0-9]\".*font="\2\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			# Pages
			s/^.*top=\"1120\".*height=\"13\".*>([0-9]+)<\/text>$/<page>\1<\/page>/g 
#			s/^<page number=\"([0-9]+)\" .*>$/<page>\1<\/page>/g
			# Program
...
#		_xmlConvert
	...
	done < src/sections_2000_2001_group_2
