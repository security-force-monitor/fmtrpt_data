#!/bin/bash
#
# Parsing FMTRPT FY 2018-2019 from PDF to a tsv
#
# tl@sfm / 2019-12-06
...
cat output/1_pdf_xml/"${y}_${t}_fmtrpt.xml" \
	...
	| sed 's/<text.*left="35".*font="3">\(.*\)<\/text>/<training>%<course_title>\1<\/course_title>/g' \
	| sed 's/<text.*left="305".*font="3">\(.*\)<\/text>/<quantity>\1<\/quantity>/g' \
	| sed 's/<text.*left="359".*font="3">\(.*\)<\/text>/<location>\1<\/location>/g' \
	| sed 's/<text.*left="548".*font="3">\(.*\)<\/text>/<student_unit>\1<\/student_unit>/g' \
	| sed 's/<text.*left="549".*font="3">\(.*\)<\/text>/<student_unit>\1<\/student_unit>/g' \
	| sed 's/<text.*left="737".*font="3">\(.*\)<\/text>/<us_unit>\1<\/us_unit>/g' \
	| sed 's/<text.*left="738".*font="3">\(.*\)<\/text>/<us_unit>\1<\/us_unit>/g' \
	| sed 's/<text.*left="926".*font="3">\(.*\)<\/text>/<total_cost>\1<\/total_cost>/g' \
	| sed 's/<text.*left="927".*font="3">\(.*\)<\/text>/<total_cost>\1<\/total_cost>/g' \
	| sed 's/<text.*left="1007".*font="3">\(.*\)<\/text>/<start_date>\1<\/start_date>/g' \
	| sed 's/<text.*left="1008".*font="3">\(.*\)<\/text>/<start_date>\1<\/start_date>/g' \
	| sed 's/<text.*left="1088".*font="3">\(.*\)<\/text>/<end_date>\1<\/end_date>%<\/training>/g' \
	| sed 's/<text.*left="1089".*font="3">\(.*\)<\/text>/<end_date>\1<\/end_date>%<\/training>/g' \
	| tr '%' '\n' \
	| grep -v -e "^<text" \
...
# Todo:
# - clean up that sed etc
# - improve readability
# - err functions, anyone?
#
# Possibly redundant tabbing from 54ff, keeping here just in case
#
#	| sed '/country/s/^/ /g' \
#	| sed '/<c_name/s/^/  /g' \
#	| sed '/program/s/^/   /g' \
