#!/bin/bash
#
# Parsing FMTRPT FY 2003-2004 (all sections except Africa) from PDF to a tsv
...
_cleanXML () {
	...
	#    the input variables ($nb) and use awk to insert it.
	...
	| sed -E '{
			...
			s/^.*left=\"270\".*font=\"3\">(.*)<\/text>$/<location>\1<\/location>/g
			s/^.*left=\"289\".*font=\"3\">(.*)<\/text>$/<location>\1<\/location>/g
			s/^.*left=\"472\".*font=\"3\">(.*)<\/text>$/<student_unit>\1<\/student_unit>/g
			s/^.*left=\"473\".*font=\"3\">(.*)<\/text>$/<student_unit>\1<\/student_unit>/g
			s/^.*left=\"486\".*font=\"3\">(.*)<\/text>$/<student_unit>\1<\/student_unit>/g
			s/^.*left=\"657\".*font=\"3\">(.*)<\/text>$/<us_unit>\1<\/us_unit>/g
			s/^.*left=\"668\".*font=\"3\">(.*)<\/text>$/<us_unit>\1<\/us_unit>/g
			s/^.*left=\"675\".*font=\"3\">(.*)<\/text>$/<us_unit>\1<\/us_unit>/g
			s/^.*left=\"682\".*font=\"3\">(.*)<\/text>$/<us_unit>\1<\/us_unit>/g
			s/^.*left=\"914\".*font=\"3\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
...
#		_xmlConvert
	...
	done < src/fmtrpt_fy_2003_2004_other_sections
