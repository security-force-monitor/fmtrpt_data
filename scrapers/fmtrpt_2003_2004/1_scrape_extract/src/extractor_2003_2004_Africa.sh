#!/bin/bash
#
# Parsing FMTRPT FY 2003-2004 Africa section from PDF to a tsv
...
_cleanXML () {
	...
	| sed -E '{
			s/^.*left=\"54\".*font=\"5\">(.*)<\/text>$/<course_title>\1<\/course_title>/g
			s/^.*left=\"249\".*font=\"5\">(.*)<\/text>$/<quantity>\1<\/quantity>/g 
			s/^.*left=\"255\".*font=\"5\">(.*)<\/text>$/<quantity>\1<\/quantity>/g 
			s/^.*left=\"268\".*font=\"5\">(.*)<\/text>$/<location>\1<\/location>/g
			s/^.*left=\"473\".*font=\"5\">(.*)<\/text>$/<student_unit>\1<\/student_unit>/g
			s/^.*left=\"486\".*font=\"5\">(.*)<\/text>$/<student_unit>\1<\/student_unit>/g
			s/^.*left=\"657\".*font=\"5\">(.*)<\/text>$/<us_unit>\1<\/us_unit>/g
			s/^.*left=\"928\".*font=\"5\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			s/^.*left=\"934\".*font=\"5\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			s/^.*left=\"940\".*font=\"5\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			s/^.*left=\"949\".*font=\"5\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			s/^.*left=\"960\".*font=\"5\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			s/^.*left=\"986\".*font=\"5\">(.*)<\/text>$/<start_date>\1<\/start_date>/g
			s/^.*left=\"989\".*font=\"5\">(.*)<\/text>$/<start_date>\1<\/start_date>/g
			s/^.*left=\"992\".*font=\"5\">(.*)<\/text>$/<start_date>\1<\/start_date>/g
			s/^.*left=\"1063\".*font=\"5\">(.*)<\/text>$/<end_date>\1<\/end_date>/g
			s/^.*left=\"1066\".*font=\"5\">(.*)<\/text>$/<end_date>\1<\/end_date>/g
			s/^.*left=\"1069\".*font=\"5\">(.*)<\/text>$/<end_date>\1<\/end_date>/g
			s/^.*left=\"577\".*font=\"9\"><b> (IV-.*)<\/b><\/text>$/<page>\1<\/page>/g
			s/^.*left=\"570\".*font=\"9\"><b> (IV-.*)<\/b><\/text>$/<page>\1<\/page>/g
						}' \
	| grep -v -e "<text.*text>$" \
	| sed '{
