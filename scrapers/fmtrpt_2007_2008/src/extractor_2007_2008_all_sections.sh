...
cat output/1_pdf_xml/"${y}_${t}_fmtrpt.xml" \
	...
	| sed 's/<text.*left="54".*font="0"><b>\(.*\) <\/b><\/text>/<program name="\1">%<p_name>\1<\/p_name>/g' \
	| sed 's/<text.*left="54".*font="2">\(.*\)<\/text>/<training>%<course_title>\1<\/course_title>/g' \
	| sed 's/<text.*left="270".*font="2">\(.*\)<\/text>/<quantity>\1<\/quantity>/g' \
	| sed 's/<text.*left="324".*font="2">\(.*\)<\/text>/<location>\1<\/location>/g' \
	| sed 's/<text.*left="513".*font="2">\(.*\)<\/text>/<student_unit>\1<\/student_unit>/g' \
	| sed 's/<text.*left="891".*font="2">\(.*\)<\/text>/<total_cost>\1<\/total_cost>/g' \
	| sed 's/<text.*left="972".*font="2">\(.*\)<\/text>/<start_date>\1<\/start_date>/g' \
	| sed 's/<text.*left="1053".*font="2">\(.*\)<\/text>/<end_date>\1<\/end_date>%<\/training>/g' \
	| tr '%' '\n' \
	| grep -v -e "^<text" \
...
xml sel -T -t -m "//training" -v "concat(ancestor::country/@name,'	',ancestor::program/@name,'	',course_title,'	','US unit not specified in 2007-2008 report','	',student_unit,'	',start_date,'	',end_date,'	',location,'	',quantity,'	',total_cost)" -n output/4_xml_dedup/"${y}_${t}_fmtrpt_dedup.xml" \
	| sed 's/ \{2,\}/ /g ; s/	 /	/g ; s/ 	/	/g' \
...
# Some sed that shouldn't be there; keeping just in case I was wrong
# Reinsert to 61ff if needed:
#	| sed '/country/s/^/ /g' \
#	| sed '/<c_name/s/^/  /g' \
#	| sed '/program/s/^/   /g' \
#	| sed '/p_name/s/^/     /g' \
