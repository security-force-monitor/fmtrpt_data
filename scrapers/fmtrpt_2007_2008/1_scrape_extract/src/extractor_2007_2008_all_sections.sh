#!/bin/bash
#
# Parsing FMTRPT FY 2007-2008 from PDF to a tsv
...
_cleanXML () {
	...
	# - Insert placeholder <training> item in countries where none was reported, along with explanation.
	...
	| grep -v -e "FY 2007 Program Totals" \
	| grep -v -e "FY 2007 Totals" \
	| sed '{
		s/<text.*number="\(.*\)" position.*$/<page>\1<\/page>/g
		s/<text.*left="54".*font="1">.*<b>\(.*\) <\/b>.*<\/text>/<country name="\1">%<c_name>\1<\/c_name>/g
		s/<text.*left="54".*font="0"><b>\(.*\) <\/b><\/text>/<program name="\1">%<p_name>\1<\/p_name>/g
		s/<text.*left="54".*font="2">\(.*\)<\/text>/<training>%<course_title>\1<\/course_title>/g
		s/<text.*left="270".*font="2">\(.*\)<\/text>/<quantity>\1<\/quantity>/g
		s/<text.*left="324".*font="2">\(.*\)<\/text>/<location>\1<\/location>/g
		s/<text.*left="513".*font="2">\(.*\)<\/text>/<student_unit>\1<\/student_unit>/g
		s/<text.*left="891".*font="2">\(.*\)<\/text>/<total_cost>\1<\/total_cost>/g
		s/<text.*left="972".*font="2">\(.*\)<\/text>/<start_date>\1<\/start_date>/g
		s/<text.*left="1053".*font="2">\(.*\)<\/text>/<end_date>\1<\/end_date>%<\/training>/g
		}' \
	| tr '%' '\n' \
	| grep -v -e "^<text"  \
	| perl -00pe '
...
_generateOutput () {
	...
	xml sel -T -t -m "//training" -v "concat(ancestor::country/@name,'	',ancestor::program/@name,'	',course_title,'	','US unit not specified in 2007-2008 report','	',student_unit,'	',start_date,'	',end_date,'	',location,'	',quantity,'	',total_cost,'	',page_number)" -n output/"${r}"/4_xml_dedup/"${y}_${t}_fmtrpt_dedup.xml" \
	| sed 's/ \{2,\}/ /g ; s/	 /	/g ; s/ 	/	/g' \
...
#		_extractPagesXmlConvert # uses just pdftohtml rather than ghostscript step
	...
	done < src/fmtrpt_fy_2007_2008_sections
