#!/bin/bash
#
# Parsing FMTRPT FY 2010-2011 from PDF to a tsv
...
_cleanXML () {
	...
	| sed '{
		...
		s/<text.*left="54".*font="1"><b>\(.*\) <\/b><\/text>/<country name="\1">%<c_name>\1<\/c_name>/
		s/<text.*left="54".*font="2"><b>\(.*\) <\/b><\/text>/<program name="\1">%<p_name>\1<\/p_name>/
		s/<text.*left="54".*font="3">\(.*\)<\/text>/<training>%<course_title>\1<\/course_title>/
		s/<text.*left="351".*font="3">\(.*\)<\/text>/<quantity>\1<\/quantity>/
		s/<text.*left="405".*font="3">\(.*\)<\/text>/<location>\1<\/location>/
		s/<text.*left="648".*font="3">\(.*\)<\/text>/<student_unit>\1<\/student_unit>/
		s/<text.*left="891".*font="3">\(.*\)<\/text>/<total_cost>\1<\/total_cost>/
		s/<text.*left="972".*font="3">\(.*\)<\/text>/<start_date>\1<\/start_date>/
		s/<text.*left="1053".*font="3">\(.*\)<\/text>/<end_date>\1<\/end_date>%<\/training>/
		}' \
	| tr '%' '\n' \
	| grep -v -e "^<text"  \
	| gawk 'BEGIN { FS = "" ;page = "" }
...
_generateOutput () {
	...
	# IMPORTANT: in this 2010 report, the US unit is not specified, so we print some placeholder text in the xmlstarlet code instead of extracting us_unit

	xml sel -T -t -m "//training" -v "concat(ancestor::country/@name,'	',ancestor::program/@name,'	',course_title,'	','US unit not specified in 2010-2011 report','	',student_unit,'	',start_date,'	',end_date,'	',location,'	',quantity,'	',total_cost,'	',page_number)" -n output/"${r}"/4_xml_dedup/"${y}_${t}_fmtrpt_dedup.xml" \
	| sed 's/ \{2,\}/ /g ; s/	 /	/g ; s/ 	/	/g' \
...
#		_extractPagesXmlConvert # uses just pdftohtml rather than ghostscript step
	...
	done < src/fmtrpt_fy_2010_2011_sections
