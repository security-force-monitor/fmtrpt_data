#!/bin/bash
#
# Parsing  FMTRPT FY 2001-2002 from PDF to a tsv (Newly Independent States)
#
# tl@sfm / 2017-04-02
...
# This script just deals with the Newly Independent States PDf from 2001-2002, as it has a foible
# Loop the extractor using a file specifying page name, report year and section as variables
# Remove some rows that detail items from countries outside of the region, and are also no closed items (so can't be closed as XML)
...
cat output/1_pdf_xml/"${y}_${t}_fmtrpt.xml" \
	...
	| sed -E '{
			...
			s/^.*left=\"244\".*font=\"2\">(.*)<\/text>$/<location>\1<\/location>/g
			s/^.*left=\"249\".*font=\"2\">(.*)<\/text>$/<location>\1<\/location>/g
			s/^.*left=\"250\".*font=\"2\">(.*)<\/text>$/<location>\1<\/location>/g
			s/^.*left=\"255\".*font=\"2\">(.*)<\/text>$/<location>\1<\/location>/g
			s/^.*left=\"266\".*font=\"2\">(.*)<\/text>$/<location>\1<\/location>/g
