#!/bin/bash
#
# Parsing FMTRPT FY 2001-2002 Cluster One sections from PDF to a tsv
...
_cleanXML () {
	...
	| case "${t}" in
		...
		"East_Asia_and_Pacific")
			sed '11,37d ; 4190,4313d'
			;;
		"South_Asia")
			sed '11,27d ; 1375,1482d'
			;;
		esac \
	| grep  -e "^<text" \
	...
	| sed -E '{
			...
			s/^.*left=\"255\".*font=\"3\">([0-9]+) (.*)<\/text>$/<quantity>\1<\/quantity>\n<location>\2<\/location>/g
			s/^.*left=\"244\".*font=\"3\">([0-9]+) (.*)<\/text>$/<quantity>\1<\/quantity>\n<location>\2<\/location>/g
			s/^.*left=\"249\".*font=\"3\">([0-9]+) (.*)<\/text>$/<quantity>\1<\/quantity>\n<location>\2<\/location>/g
			s/^.*left=\"250\".*font=\"3\">([0-9]+) (.*)<\/text>$/<quantity>\1<\/quantity>\n<location>\2<\/location>/g
			s/^.*left=\"266\".*font=\"3\">([0-9]+) (.*)<\/text>$/<quantity>\1<\/quantity>\n<location>\2<\/location>/g
			s/^.*left=\"244\".*font=\"3\">([0-9]+)<\/text>$/<quantity>\1<\/quantity>/g
			s/^.*left=\"249\".*font=\"3\">([0-9]+)<\/text>$/<quantity>\1<\/quantity>/g
			s/^.*left=\"250\".*font=\"3\">([0-9]+)<\/text>$/<quantity>\1<\/quantity>/g
			s/^.*left=\"255\".*font=\"3\">([0-9]+)<\/text>$/<quantity>\1<\/quantity>/g
			s/^.*left=\"266\".*font=\"3\">([0-9]+)<\/text>$/<quantity>\1<\/quantity>/g
		...
		#	s/^.*left=\"266\".*font=\"3\">(.*)<\/text>$/<location>\1<\/location>/g
			...
			s/^.*left=\"577\".*font=\"2\"><b> (IV-.*)<\/b><\/text>$/<page>\1<\/page>/g
			s/^.*left=\"570\".*font=\"2\"><b> (IV-.*)<\/b><\/text>$/<page>\1<\/page>/g
