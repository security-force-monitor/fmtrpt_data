#!/bin/bash
#
# Parsing FMTRPT FY 2001-2002 Cluster Two sections from PDF to a tsv
...
#          2024-08-30 Adapted to handle mis-placed training items
...
_cleanXML () {
	...
	# - repair source XML to handle training items from other Regions included in PDFs of other regions
	...
	| sed -E '{
			...
			s/>APR<//g
			s/>INFORMATION MANAGEMENT </>INFORMATION MANAGEMENT APR</g
			s/>SCHOOL<//g
			s/>ACADEMIC INSTRUCTOR </>ACADEMIC INSTRUCTOR SCHOOL</g
			s/>RP<//g
			s/>COMPUTER\/AUTOMATION SYS </>COMPUTER\/AUTOMATION SYS RP</g
			s/>CRS<//g
			s/>MAINTENANCE MANAGEMENT </>MAINTENANCE MANAGEMENT CRS</g
			s/>MSM<//g
			s/>METROLOGY ENG GAS FLOW </>METROLOGY ENG GAS FLOW MSM</g
			s/>PROGRAM<//g
			s/>QUALITY MANAGEMENT </>QUALITY MANAGEMENT PROGRAM</g
			s/>SYSTEM<//g
			s/>COMMAND AND CONTROL </>COMMAND AND CONTROL SYSTEM</g
			s/>CRS-SP<//g
			s/>RESOURCE MANAGEMENT </>RESOURCE MANAGEMENT CRS-SP</g
			s/>CG9<//g
			s/>MTT OUTBOARD MTR MAINT </>MTT OUTBOARD MTR MAINT CG9>/g
			s/>N<//g
			s/>AN-CONCEPTS\/CONFIGURATIO </>AN-CONCEPTS\/CONFIGURATION</g
						}' \
	| gawk -v nb="$nb" 'BEGIN {print "<page>IV-"nb-1"</page>"};{print}' \
	| sed -E '{
			s/^.*left=\"54\".*font=\"2\">(.*)<\/text>$/<course_title>\1<\/course_title>/g
			s/^.*left=\"525\".*font=\"2\">(.*)<\/text>$/<quantity>\1<\/quantity>/g
			s/^.*left=\"255\".*font=\"2\">([0-9]+) (.*)<\/text>$/<quantity>\1<\/quantity>\n<location>\2<\/location>/g
			s/^.*left=\"244\".*font=\"2\">([0-9]+) (.*)<\/text>$/<quantity>\1<\/quantity>\n<location>\2<\/location>/g
			s/^.*left=\"249\".*font=\"2\">([0-9]+) (.*)<\/text>$/<quantity>\1<\/quantity>\n<location>\2<\/location>/g
			s/^.*left=\"250\".*font=\"2\">([0-9]+) (.*)<\/text>$/<quantity>\1<\/quantity>\n<location>\2<\/location>/g
			s/^.*left=\"266\".*font=\"2\">([0-9]+) (.*)<\/text>$/<quantity>\1<\/quantity>\n<location>\2<\/location>/g
			s/^.*left=\"244\".*font=\"2\">([0-9]+)<\/text>$/<quantity>\1<\/quantity>/g
			s/^.*left=\"249\".*font=\"2\">([0-9]+)<\/text>$/<quantity>\1<\/quantity>/g
			s/^.*left=\"250\".*font=\"2\">([0-9]+)<\/text>$/<quantity>\1<\/quantity>/g
			s/^.*left=\"255\".*font=\"2\">([0-9]+)<\/text>$/<quantity>\1<\/quantity>/g
			s/^.*left=\"266\".*font=\"2\">([0-9]+)<\/text>$/<quantity>\1<\/quantity>/g
			s/^.*left=\"472\".*font=\"2\">(.*)<\/text>$/<student_unit>\1<\/student_unit>/g
			s/^.*left=\"473\".*font=\"2\">(.*)<\/text>$/<student_unit>\1<\/student_unit>/g
			s/^.*left=\"657\".*font=\"2\">(.*)<\/text>$/<us_unit>\1<\/us_unit>/g
			s/^.*left=\"920\".*font=\"2\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			s/^.*left=\"928\".*font=\"2\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			s/^.*left=\"929\".*font=\"2\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			s/^.*left=\"934\".*font=\"2\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			s/^.*left=\"940\".*font=\"2\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			s/^.*left=\"949\".*font=\"2\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			s/^.*left=\"955\".*font=\"2\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			s/^.*left=\"960\".*font=\"2\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			s/^.*left=\"961\".*font=\"2\">(.*)<\/text>$/<total_cost>\1<\/total_cost>/g
			s/^.*left=\"992\".*font=\"2\">(.*)<\/text>$/<start_date>\1<\/start_date>/g
			s/^.*left=\"995\".*font=\"2\">(.*)<\/text>$/<start_date>\1<\/start_date>/g
			s/^.*left=\"998\".*font=\"2\">(.*)<\/text>$/<start_date>\1<\/start_date>/g
			s/^.*left=\"1068\".*font=\"2\">(.*)<\/text>$/<end_date>\1<\/end_date>/g
			s/^.*left=\"1069\".*font=\"2\">(.*)<\/text>$/<end_date>\1<\/end_date>/g
			s/^.*left=\"1071\".*font=\"2\">(.*)<\/text>$/<end_date>\1<\/end_date>/g
			s/^.*left=\"1072\".*font=\"2\">(.*)<\/text>$/<end_date>\1<\/end_date>/g
			s/^.*left=\"1074\".*font=\"2\">(.*)<\/text>$/<end_date>\1<\/end_date>/g
			s/^.*left=\"563\".*font=\"4\"><b> (IV-.*)<\/b><\/text>$/<page>\1<\/page>/g
			s/^.*left=\"570\".*font=\"4\"><b> (IV-.*)<\/b><\/text>$/<page>\1<\/page>/g
						}' \
	| grep -v -e "<text.*text>$" \
	| sed '{
