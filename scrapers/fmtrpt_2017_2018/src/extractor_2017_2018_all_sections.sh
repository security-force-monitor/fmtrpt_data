...
# First version of this script, so forgive the ugliness
...
xml sel -T -t -m "//training" -v "concat(ancestor::country/@name,'	',ancestor::program/@name,'	',course_title,'	',us_unit,'	',student_unit,'	',start_date,'	',end_date,'	',location,'	',quantity,'	',total_cost)" -n output/4_xml_dedup/"${y}_${t}_fmtrpt_dedup.xml" \
	...
	done < src/fmtrpt_fy_2017_2018_sections

# To do:
# - all that sed,awk,perl ... into single processes?
# - overall readability is low
# - some functions perhaps?

# Some unnecessary tabbing added in early versions
