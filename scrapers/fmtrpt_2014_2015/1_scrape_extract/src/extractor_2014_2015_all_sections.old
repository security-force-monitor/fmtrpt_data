...
xml sel -T -t -m "//training" -v "concat(ancestor::country/@name,'	',ancestor::program/@name,'	',course_title,'	',us_unit,'	',student_unit,'	',start_date,'	',end_date,'	',location,'	',quantity,'	',total_cost)" -n output/4_xml_dedup/"${y}_${t}_fmtrpt_dedup.xml" \
	...
	done < src/fmtrpt_fy_2014_2015_sections


# To do:
# - loads of cleanup on the sed, grep and awk statements
# Keeping these here just in case:
# 56-68 yanked - possibility unnecessary seds adding in unnecessary tabbing
	#| sed '/country/s/^/ /g' \
	#| sed '/<c_name/s/^/  /g' \
	#| sed '/program/s/^/   /g' \
	#| sed '/p_name/s/^/     /g' \
