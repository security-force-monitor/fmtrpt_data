...
#	2024-09-20 A few additional country name permutations to fix
#		   Tweaks to start and end date cleanup functions
#
...
i="fmtrpt_1999_2000_202407121057.tsv"
...
gawk '			BEGIN { FS=OFS="\t"} \
			...
			{if ($2=="\"Netherlands Antilles (NE)\"") $3="\"ANT\""} \
...
gawk '			BEGIN { FS=OFS="\t" } \
			{ if ($2 ~ /^"1999_/) $1="\"4659b43b-be9a-4830-a653-ab7c51c20c5f\""} \
...
# Test for presence of "not included", and operate on anything other than the header

echo "Dealing with errors in the 2009-2010 dates, which are in the same column"

gawk 'BEGIN	{ FS=OFS="\t"};							\
		{ if (NR==1) print  ; 						\
		 else if ( NR>1 && $9 ~ /not included/ ) print ;			\
		  else if ( NR>1 && $9 !~ /not included/ )			\
		    {	gsub(/"/,"",$9) ;					\
			split($9,date," ") ;					\
			s=date[1] ;						\
			e=date[2] ;						\
...
# Sometimes. this date is deliberately not provided, so we  reflect that

gawk 'BEGIN 	{ FS=OFS="\t"}; 						\
		{ if ( NR>1 && $9 ~ /not included/ ) $10 = $9 ;			\
		else if (NR>1 && $9 !~/not included/)				\
		{ 	gsub(/"/,"",$9) ;					\
			split($9,date,"/") ;					\
				y=date[3] ;					\
...
gawk 'BEGIN 	{ FS=OFS="\t"}; 						\
		{ if ( NR>1 && $11 ~ /not included/ )  $12 = $11 ;	 	\
		else if ( NR>1 && $11 !~/not included/) 			\
		      { gsub(/"/,"",$11) ;					\
			split($11,date,"/") ;					\
				y=date[3] ;					\
				d=date[2] ;					\
...
gawk 	'BEGIN { FS=OFS="\t" }	;											\
	 ...
	 { if (NR > 1 && $17 ~ /\.htm/) $17 = "https://2009-2017.state.gov/t/pm/rls/rpt/fmtrpt/2007/" $17  ; 	\
	   ...
	   else if (NR > 1 && $1 ~ /4659b43b-be9a-4830-a653-ab7c51c20c5f/ ) $17 = "https://1997-2001.state.gov/global/arms/fmtrain/" $17 ".html" ;\
...
gawk 'BEGIN { FS=OFS="\t"}; { $17 = $17 FS "\"date_first_seen\""; print $0 }' "notes/17_${i}" \
	| gawk 'BEGIN { FS=OFS="\t"} ; 								\
		{if ($1 ~ /4659b43b-be9a-4830-a653-ab7c51c20c5f/) $18 = "\"2000-03-01\""}	\
...
gawk 'BEGIN { FS=OFS="\t"} ;										\
		{if ($1 ~ /4659b43b-be9a-4830-a653-ab7c51c20c5f/) $19 = $19 FS "\"2024-09-20\"" }	\
		{if ($1 ~ /7713f87d-605c-4563-acc2-0072d7dcc957/) $19 = $19 FS "\"2024-09-20\"" }	\
