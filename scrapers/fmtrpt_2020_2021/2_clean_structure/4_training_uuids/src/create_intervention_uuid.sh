#!/bin/bash
#
# Mint UUID for training intervention entity
# 
# tl@sfm	2024-06-19 updated to add newly scraped page_number
...
_mintTrainingUUID () {
	...
	# Takes ~5 minutes to complete uuid generation (for full dataset)
	# Only really need to run this once to mint the uuids. 

	gawk -F"\t" '("uuidgen" | getline uuid) > 0 {print "\""tolower(uuid)"\"" FS $0} {close("uuidgen")}'

}

_makeEmptyIDCol () {

	# In some circumstances we may want to make a version with an empty
	# column for the training ID
...
#	Note: longhand printout just reoderes the columns a bit
...
_main () {
	...
	_renameHeaders notes/"$blankidcol" > output/"$data"
