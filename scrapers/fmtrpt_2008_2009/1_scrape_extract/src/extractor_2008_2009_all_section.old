...
cat output/1_pdf_xml/"${y}_${t}_fmtrpt.xml" \
	...
	| sed 's/<text.*left="54".*font="2"><i><b>\(.*\) <\/b><\/i><\/text>/<program name="\1">%<p_name>\1<\/p_name>/g' \
...
xml sel -T -t -m "//training" -v "concat(ancestor::country/@name,'	',ancestor::program/@name,'	',course_title,'	','US unit not specified in 2008-2009 report','	',student_unit,'	',start_date,'	',end_date,'	',location,'	',quantity,'	',total_cost)" -n output/4_xml_dedup/"${y}_${t}_fmtrpt_dedup.xml" \
	| sed 's/ \{2,\}/ /g ; s/	 /	/g ; s/ 	/	/g' \
